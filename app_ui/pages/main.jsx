...
export default function Main() {
  ...
  const removeFolder = (indexToRemove, event) => {
    ...
    if (window.confirm(`'${folderToRemove.name}' 폴더를 정말 삭제하시겠습니까?`)) {
      const updatedFolders = folders.filter((_, index) => index !== indexToRemove);
      ...
      <div className={styles.grayBackground}>
        ...
        {folders.length === 0 ? (
          ...
          <div className={styles.folderSection}>
            <div className={`${styles.folderList} ${folders.length > 0 && folders.length % 12 === 0 ? styles['columns-4'] : (folders.length > 0 && folders.length % 3 === 0 ? styles['columns-3'] : '')}`.trim()}>
